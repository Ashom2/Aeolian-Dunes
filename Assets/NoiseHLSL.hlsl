...
float Combinations(uint n, uint k)
{
    //might be faster to do manually https://en.wikipedia.org/wiki/Combination
    return Factorial(n) / (Factorial(k) * Factorial(n - k));
}

//i guess pow doesnt work in glsl FFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFUUU!!!
float Power(float x, uint y)
{
    float val = 1;
    for (uint i = 0; i < y; i++) 
    {
...
//for everything else
float Layered4DNoise(float4 Coordinates, uint seed, float Scale, uint Octaves, float Persistence, float Lacunarity)
{
    ...
    //float4 samplePoint = float4(coordinates.x, coordinates.y, coordinates.z, offset);

    for (uint i = 0; i < Octaves; i++) 
    {
        float noise = cnoise(Coordinates * frequency + Random(seed + i));
        height += (0.5 * noise + 0.5) * amplitude; //normalized
        //height += (1 - abs(noise)) * amplitude; //jagged
...
//for shader graph
void Layered4DNoise_float(float4 Coordinates, uint seed, float Scale, uint Octaves, float Persistence, float Lacunarity, out float Out)
{
    Out = Layered4DNoise(Coordinates, seed, Scale, Octaves, Persistence, Lacunarity);
}

float NormalizedLayered4DNoise(float4 Coordinates, uint seed, float Scale, uint Octaves, float Persistence, float Lacunarity)
{
    float height = Layered4DNoise(Coordinates, seed, Scale, Octaves, Persistence, Lacunarity);

    if (Persistence != 1)
    {
        float geometricSum = (1 - pow(Persistence, Octaves)) / (1 - Persistence);

        height *= Octaves / geometricSum;
    }

    float summation = 0;
    for (uint k = 0; k <= Octaves; k++) 
    {
        //can be optimised using precomputed values
        // pow(-1, k) doesnt work!!!!! 
        summation += Power(-1, k) * Combinations(Octaves, k) * sign(height - k) * Power(height - k, Octaves);
        
    }

    return 0.5 + (0.5 / Factorial(Octaves)) * summation;
        //Out = lerp(height / Octaves, 0.5 + (0.5 / Factorial(Octaves)) * summation, Persistence);
}
//for shader graph
void NormalizedLayered4DNoise_float(float4 Coordinates, uint seed, float Scale, uint Octaves, float Persistence, float Lacunarity, out float Out)
{
    Out = NormalizedLayered4DNoise(Coordinates, seed, Scale, Octaves, Persistence, Lacunarity);
}

float2 CurlNoise(float4 Coordinates, uint seed, float Scale, uint Octaves, float Persistence, float Lacunarity)
{
    float x = Layered4DNoise(Coordinates, Random(seed + 2), Scale, Octaves, Persistence, Lacunarity);
    float y = Layered4DNoise(Coordinates, Random(seed + 4), Scale, Octaves, Persistence, Lacunarity);
